...
client.on('messageCreate', async (message) => {
  ...
  if (content.startsWith('/addgame')) {
    ...
    const repoPath = path.resolve(__dirname, '../OST-Manifest-Store');
    const keysPath = path.resolve(__dirname, '../depotkeys.json');
    const jsonPath = path.join(repoPath, 'games.json');

    if (!fs.existsSync(repoPath) || !fs.existsSync(keysPath)) {
      return message.reply('❌ Lỗi hệ thống máy chủ, vui lòng báo Admin.');
    }

    try {
      // 0. Kiểm tra xem game đã tồn tại chưa để chống trùng lặp
      ...
      if (isExist) {
        return message.reply(`⚠️ **Game này đã có sẵn trên Chợ VIP rồi!**\nNếu game có bản cập nhật mới và bạn muốn Add lại, vui lòng xóa thủ công file trên Github trước nhé.`);
      }

      const loadingMsg = await message.reply(`🔄 Đang xử lý dữ liệu cho AppID **${appId}**...`);
      
      // 1. Lấy thông tin game từ Steam API
      ...
      if (steamData && steamData.success) {
        ...
        // Backup: Nếu Steam không có, lấy tên game từ tin nhắn người dùng gõ
      ...
      // 3. Tự động sinh file Lua nếu người dùng KHÔNG đính kèm file .lua
      if (!luaUrl) {
        const keysData = JSON.parse(fs.readFileSync(keysPath, 'utf8'));

        if (attachedDepotIds.length > 0) {
          // Bán tự động: Dò key dựa trên file manifest đính kèm
          ...
          // Tự động hoàn toàn: Quét 30 ID liền kề
          for (let i = 1; i <= 30; i++) {
        ...
        if (foundKeys === 0) {
          return loadingMsg.edit(`❌ **Chưa có dữ liệu Depot Key.**\nHiện tại kho tàng chưa có khóa giải mã cho tựa game **${gameName}**. Anh vui lòng tải thủ công file \`.lua\` lên nhé!`);
        }
      ...
      const gitCmd = `cd "${repoPath}" && git add . && git commit -m "Add/Update ${gameName}" && git push`;
      exec(gitCmd, (error) => {
        if (error) {
          console.error(`Git push error: ${error}`);
          return loadingMsg.edit(`⚠️ Đã lưu vào máy chủ nhưng đồng bộ Github thất bại.`);
        }
